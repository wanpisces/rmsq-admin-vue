...
import { RequestQuestionSubjectList, RequestQuestionnaireUser, RequestQuestionnaireSubjectFill } from "@/fetch/modules/service.js";
...
export default {
  ...
  methods: {
    ...
    exportVoteList() {
      //  config.baseUrl +
      window.location.href = "api/questionnaireAdmin/questionnaireExport" + "?token=" + getToken() + "&questionnaire_id=" + this.questionnaire_id;
    },
    // 获取填空题详情
    getFillBlankDetail() {
      ...
      RequestQuestionnaireSubjectFill(_arg).then(res => {
        console.log("RequestQuestionnaireSubjectFill", res);
        if (res.code == 2000) {
    ...
    exportDetailText() {
      // config.baseUrl +
      window.location.href = "api/questionnaireAdmin/subjectFillExport" + "?token=" + getToken() + "&subject_id=" + this.fillBlankForm.subject_id;
    },
    // 用户答题详情
    toUserInfo(userID) {
      this.$router.push({
        path: "viewQuestioUser" + this.urlLastId,
    ...
    backToList() {
      ...
      this.$store.dispatch("delVisitedViews", {
        path: "/questionModule/viewQuestion" + this.urlLastId
      });
    }
  }
};
