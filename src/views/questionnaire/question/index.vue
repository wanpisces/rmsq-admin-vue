    ...
    <div>
      <el-card class="box-card">
        ...
        <div class="search-box">
          <div class="search">
            <span>输入搜索：</span>
            <el-input style="width:200px" v-model="search.q" placeholder="问卷标题"></el-input>
          </div>
          <div class="search">
            <span>状态：</span>
        ...
        <div class="table-list">
          <div class="table-title">
            ...
            <div class="table-title-right">
              ...
              <choose-tab @handleAdd="handleAdd(1)" @TimeOn="timeOn" add-title="新建问卷">
              </choose-tab>
            </div>
          </div>
          <el-table :data="tableData" border style="width: 100%" v-loading='loading' element-loading-text="拼命加载中...." element-loading-spinner="el-icon-loading" ref="table" @row-click="handleCurrentChange" @selection-change="selectionChange">
            ...
            <el-table-column label="问卷标题" align="center">
              <template slot-scope="scope">
                {{scope.row.questionnaire_title}}
              </template>
            </el-table-column>
            ...
            <el-table-column fixed="right" label="操作" width="120" align="center">
              <template slot-scope="scope">
                <span v-if="scope.row.status == 2">
                  <router-link :to="{path:'previewQuestion'+urlLastId,query:{questionnaire_id:scope.row.questionnaire_id}}">
                    <el-button type="text" size="small"> 预览 </el-button>
                  </router-link>
                  <el-button @click="changeVoteStatus(scope.row.questionnaire_id)" type="text" size="small"> 发布 </el-button>
                </span>
                <span v-else>
                  <router-link :to="{path:'viewQuestion'+urlLastId,query:{questionnaire_id:scope.row.questionnaire_id}}">
                    <el-button type="text" size="small"> 查看 </el-button>
                  </router-link>
                  <el-button @click="changeVoteStatus(scope.row.questionnaire_id)" type="text" size="small"> 取消发布 </el-button>
                </span>
                <el-button v-if="scope.row.status == 2" @click="handleAdd(2,scope.row.questionnaire_id)" type="text" size="small"> 编辑 </el-button>
                <!-- <div class="el-icon-wrapper">
                  <span v-if="scope.row.status != 2">
                    ...
                    <i class="icon-quxiaofabu" title="取消发布" @click="changeVoteStatus(scope.row.questionnaire_id)"></i>
                  </span>
                  <span v-if="scope.row.status == 2">
                    <router-link :to="{path:'previewQuestion'+urlLastId,query:{questionnaire_id:scope.row.questionnaire_id}}">
                      <el-button type="text" size="small"> 预览 </el-button>
                    ...
                    <i class="icon-fabu" title="发布" @click="changeVoteStatus(scope.row.questionnaire_id)"></i>
                    <i class="el-icon-edit" title="编辑" @click="handleAdd(2,scope.row.questionnaire_id)"></i>
                  </span>
                </div> -->
              </template>
            </el-table-column>
...
import { RequestServicePublicDetail, RequestQuestionnaire, RequestQuestionnaireRemove, RequestServiceState, RequestQuestionnaireState } from "@/fetch/modules/service.js";
...
export default {
  ...
  methods: {
    ...
    newList(params) {
      ...
      RequestQuestionnaire(_arg).then(res => {
        if (res.code == 2000) {
          this.tableData = res.data.list;
          this.count = res.data.total;
        } else {
        }
        console.log("__RequestQuestionnaire__", res);
      });
    },
    // 切换发布状态
    changeVoteStatus(_questionnaire_id) {
      RequestQuestionnaireState({
        ...
        console.log("__RequestQuestionnaireState__", res);
      });
    },
    // 是否在用户端显示
    ...
    // 新建问卷
    handleAdd(type, _question_id) {
      if (type == 1) {
        this.$router.push({
          path: "addQuestion" + this.urlLastId
        });
      } else {
        this.$router.push({
          path: "editQuestion" + this.urlLastId,
          query: { questionnaire_id: _question_id }
        });
      }
    },
    // 时间排序切换
    ...
    tabelDelete() {
      ...
      for (let i in this.selectionValues) {
        selectionArr.push(this.selectionValues[i].questionnaire_id);
      }
      this.$confirm("此操作将永久删除, 是否继续?", "提示", {
        ...
        .then(() => {
          RequestQuestionnaireRemove({ questionnaire_id: selectionArr }).then(res => {
            self.newList();
          });
        })
        .catch(() => {
