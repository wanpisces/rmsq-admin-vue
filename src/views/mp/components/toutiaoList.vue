  ...
  <div class="info-container">
    <el-card class="box-card">
      ...
      <div class="search-box">
        <div class="search">
          <span>输入搜索：</span>
          <el-input style="width:200px" placeholder="头条账号" v-model="objSearch.q"></el-input>
        </div>
        <div class="search">
    ...
    <el-card class="box-card">
      <div class="table-list">
        ...
        <el-table :data="tableData" border @selection-change="handleSelectionChange" style="width: 100%">
          ...
          <el-table-column label="头条账号" prop="toutiao_account">
          </el-table-column>
    ...
    <el-dialog :title="formTitle" :visible.sync="releaseDialog" :before-close="handleClose" :close-on-click-modal="false" width="400px">
      <el-form :model="ruleForm" :rules="rules" ref="ruleForm">
        <el-form-item label="账号：" :label-width="formLabelWidth" prop="toutiao_account">
          <el-input v-model="ruleForm.toutiao_account" auto-complete="off"></el-input>
        </el-form-item>
        <el-form-item label="密码：" :label-width="formLabelWidth" prop="toutiao_pwd">
...
import { RequestToutiaoList, RequestToutiaoAdd, RequestToutiaoEditor, RequestToutiaoDelete, RequestToutiaoDetail, RequestToutiaoCaptcha, RequestToutiaoIphoneCode, RequestToutiaoSec, RequestToutiaoByMobile } from "@/fetch/modules/public";
...
export default {
  ...
  methods: {
    ...
    getListData(args) {
      ...
      RequestToutiaoList(args).then(res => {
        if (res.code === 2000) {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          if (this.oprateStatus == 1) {
            RequestToutiaoEditor(args).then(res => {
              oprateFn(res);
            });
          } else {
            RequestToutiaoAdd(args).then(res => {
              oprateFn(res);
            });
          }
    ...
    handleSelectionChange(vals) {
      for (var i = 0; i < vals.length; i++) {
        this.selectionValues.push(vals[i].toutiao_id);
      }
      if (vals.length <= 0) {
    ...
    deleteFn(toutiao_id) {
      this.$confirm("是否永久删除?", "提示", {
        ...
        .then(() => {
          RequestToutiaoDelete({ toutiao_id: toutiao_id }).then(res => {
            var args = this.objSearch;
            // this.getListData(args);
            // this.$Message({
            //   type: "success",
