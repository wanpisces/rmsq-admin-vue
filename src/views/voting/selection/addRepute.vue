<template>
  <div class="info-container fix-content">
        ...
        <section class="phone-body">
          <el-scrollbar ref="elscrollbar" style="height:100%;">
            <div class="phone-scrollbar">
              <h5 class="word_wrap">{{form.vote_title || '评选标题' }}</h5>
              ...
              <div class="phone-content">
                  ...
                  <div v-for="(item,index) in form.subjects" :key="index" class="phone-topic">
                    ...
                    <div v-else-if="item.subject_type == 3 || item.subject_type == 4">
                      <p class="topic-item word_wrap" v-for="(item2,index2) in item.subject_items" :key="index2">
                        <i class="icon iconfont2 " :style="{color: themeColorShow}" :class="item.subject_type == 3 ? 'icon-circle' : 'icon-xuanze'"></i>{{item2.subject_item_title || '-'}}</p>
                    </div>
                ...
                <div id="componentEnd" class="pcontent-operat">
                  ...
                  <div class="operat-button no_select_text" :style="{backgroundColor: themeColorShow}">
                    <i class="icon iconfont2 icon-toupiao"></i>投票
                  </div>
    ...
    <el-card class="box-card card-middle" :body-style="{ padding: '90px 0 10px 0',minHeight: '500px', height: 'calc(100vh - 152px)', overflowY: 'auto',overflowX: 'hidden'}">
      <div slot="header" class="clearfix">
        <span>{{vote_id ? '编辑':'新建' }}评选</span>
      </div>

      <div class="card-curBody">
        <el-form ref="form" :model="form" label-width="160px">
          <el-form-item label="评选标题：" prop="vote_title" :rules="[{ required: true, message: '请填写评选标题', trigger: 'blur' }]">
            <el-input maxlength="40" placeholder="不超过40个字符！" v-model="form.vote_title" style="width:400px;"></el-input>
          </el-form-item>
          <el-form-item label="评选说明：">
            <el-input type="textarea" placeholder="不超过200个字符！" maxlength="200" v-model="form.vote_desc" style="width:400px;"></el-input>
          </el-form-item>
          <!-- div.subjects-box 添加指令 v-dragging="{ item: item, list: form.subjects, group: 'subjects', otherData: {index:index} }" 即可拖拽 -->
          <div :id="'topic'+index" class="subjects-box" tabindex="-1" v-for="(item,index) in form.subjects" :key="index">
                ...
                <div class="topicOption">
                  <div class="topic-option-box">
                    <p v-if="item.subject_type == 1 || item.subject_type == 2">
                      <em style="color:red;">* </em>图片支持、jpg、jpeg、png、gif，建议尺寸500*500
                    </p>
                    ...
                    <div v-for="(item2,index2) in item.subject_items" :key="index2" class="topic-option-item">
                      ...
                      <div v-if="item.subject_type == 1 || item.subject_type == 2">
                        <div>
                          <el-form-item :key="'index'+index+'index2_0_'+index2" label="" :prop="'subjects.'+index+'.subject_items.'+index2+'.subject_item_images[0].img_url'" :rules="[{ required: true, message: '请选择选项图片'}]">
                            <uploadFile class="uploadClick avatar-uploader" tabindex="-1" @change="handleChange" :uploadSelf="true" :maxSize="2" :paramsObj="{indexs:[index,index2],subjectType:'subject_item_images',urlType:'img_url'}">
                        ...
                        <div class="option-item-right">
                          <el-form-item :key="'index'+index+'index2_1_'+index2" label="" :prop="'subjects.'+index+'.subject_items.'+index2+'.subject_item_title'" :rules="[{ required: true, message: '请填写选项内容', trigger: 'blur' }]">
                            <el-input size="small" maxlength="20" placeholder="选项内容" v-model="item2.subject_item_title" style="width:200px;"></el-input>
                          </el-form-item>

                          <el-form-item :key="'index_'+index+'index2_2_'+index2" label="" :prop="'subjects.'+index+'.subject_items.'+index2+'.subject_item_content'" :rules="[{ required: true, message: '请填写评选人信息', trigger: 'blur' }]">
                            <el-input maxlength="400" :autosize="{ minRows: 2, maxRows: 4}" type="textarea" placeholder="评选人信息" v-model="item2.subject_item_content" style="width:200px;"></el-input>
                          </el-form-item>
                        </div>
                      ...
                      <!-- 文字 -->
                      <!-- <div v-else>
                    <el-form-item label-width="80px" :label=" '选项'+(index2+1)+'：'" :prop="'subjects.'+index+'.subject_items.'+index2+'.subject_item_title'" :rules="[{ required: true, message: '请填写选项内容', trigger: 'blur' }]">
                      <el-input size="mini" maxlength="40" placeholder="选项内容" v-model="item2.subject_item_title" style="width:220px;"></el-input>
                    </el-form-item>
          ...
          <!-- <el-form-item label="评选截止时间：" prop="end_date" :rules="[{ required: true, message: '请选择截止时间', trigger: 'blur' }]">
...
import { RequestServiceRun, RequestVoteAdd, RequestSubjectList, RequestVoteEdit, RequestVoteDetail, RequestServiceIcon, RequestSubjectRemove, RequestSubjectItemRemove, RequestServiceClassAdd, RequestformBuilderList, RequestThemeList } from "@/fetch/modules/service.js";
...
export default {
  components: { uploadFile, checkCard, ShareTemplate },
  data() {
    return {
      urlLastId: "",
      // otherInput: "", // 其他组件，姓名
  ...
  methods: {
    ...
    getVoteDetail() {
      ...
      RequestVoteDetail({
        ...
        // 活动
        //  = res.data.link_third_miniprogram || [];
          ...
          res.data.link_third_miniprogram.forEach(row => {
            this.mpActiveData.forEach(val => {
              if (row.branch_id == val.branch_id && row.service_id == val.service_id) {
                // console.log("1___________", val);
                _self.$refs.mpActiveDataTable.toggleRowSelection(val, true);
                // setTimeout(() => {
                //   this.form.activity_list.push(val);
                // }, 1);
              }
            });
          });
                ...
                if (valid) {
                  ...
                  if (!this.vote_id) {
                    RequestVoteAdd(_args).then(res => {
                      ...
                      console.log("__RequestVoteAdd__", res);
                      if (res.code == 2000) {
                        this.backToList();
    ...
    backToList() {
      const _url = !this.vote_id ? "/addRepute" : "/editRepute";
      this.$router.push({
        path: "selection" + this.urlLastId
      });
