  ...
  <div class="info-container">
    <el-card class="box-card  preview-content">
      ...
      <div class="card-body">
        ...
        <h3>
          <strong>评选标题：</strong>{{voteData.vote_title}}</h3>
        <p class="word-wrap">
          <strong>评选说明：</strong>{{voteData.vote_desc}}</p>
            ...
            <div style="padding: 18px 10px 0;" v-for="(item,index) in voteData.subjects" :key="index">
              ...
              <section v-if="item.subject_type != '0'">
                ...
                <div class="topic-option">
                  ...
                  <!-- 图文选项 -->
                  <div v-if="item.subject_type == '1' || item.subject_type == '2'" class="topic-option-box">
                    <div v-for="(item2,index2) in item.subject_items" :key="index2" class="topic-option-item">
                      <div class="img-bg-box" :style="{backgroundImage: 'url(' + item2.subject_item_images[0].img_url + ')'}">
                        <div @click="showDetail(item2.subject_item_content)">
        ...
        <p>
          <strong>是否需要匿名投票：</strong>{{voteData.allow_anonymous == 1 ? '是' : '否'}}</p>
        <p>
        ...
        <div class="preview-footer">
          ...
          <el-button @click="backToList">返回评选列表</el-button>
        </div>
    ...
    <el-dialog title="详细信息" :visible.sync="dialogVisible" width="30%">
      <p class="word-wrap">{{dialogVisibleContent}}</p>
...
<script>
import { RequestVoteDetail, RequestSubjectList, RequestVoteState } from "@/fetch/modules/service.js";
...
export default {
  ...
  methods: {
    ...
    // 显示选项详情
    showDetail(_content) {
      this.dialogVisibleContent = _content;
    ...
    backToList() {
      ...
      this.$store.dispatch("delVisitedViews", {
        path: "/voting" + this.urlLastId + "/previewReputes" + this.urlLastId
      });
    }
  }
};
