<template>
  <div class="info-container fix-content ">
        ...
        <section class="phone-body">
          <el-scrollbar ref="elscrollbar" style="height:100%;">
            <div class="phone-scrollbar">
              <h5 class="word_wrap">{{form.vote_title || '投票标题' }}</h5>
              ...
              <div class="phone-content">
                <div class="pcontent-body">
                  <div v-for="(item,index) in form.subjects" :key="index" class="phone-topic">
                    <h6 v-if="item.subject_type != 0">{{index+1}}. {{item.subject_title || "题目标题"}}
                      <span>（{{(item.subject_type == '1' || item.subject_type == '3'|| item.subject_type == '5'|| item.subject_type == '7')?'单选':'多选，最多选'+item.max_select+'项，至少选'+item.min_select+'项'}}）</span>
                    </h6>

                    <!-- 图文 -->
                    <div class="flex-between-wrap" v-if="item.subject_type == 1 || item.subject_type == 2">
                      <div class="topic-item " v-for="(item2,index2) in item.subject_items" :key="index2">
                        <div class="img-bg-box" :style="{backgroundImage: 'url(' + item2.subject_item_images[0].img_url + ')',width:'120px',height:'120px',borderRadius:'5px'}">
                        </div>
                        <!-- 选项内容 -->
                        <p class="word_wrap">
                          <i class=" iconfont2" :style="{color: themeColorShow}" :class="item.subject_type == 1 ? 'icon-circle' : 'icon-xuanze'"></i>{{item2.subject_item_title || '-'}}</p>
                      </div>
                    </div>

                    <!-- 文字 -->
                    <div v-else-if="item.subject_type == 3 || item.subject_type == 4">
                      <p class="topic-item word_wrap" v-for="(item2,index2) in item.subject_items" :key="index2">
                        <i class=" iconfont2 " :style="{color: themeColorShow}" :class="item.subject_type == 3 ? 'icon-circle' : 'icon-xuanze'"></i>{{item2.subject_item_title || '-'}}</p>
                    </div>
                    ...
                    <div class="flex-between-wrap" v-else-if="item.subject_type == 5 || item.subject_type == 6">
                      <div class="topic-item" v-for="(item2,index2) in item.subject_items" :key="index2">
                        ...
                        <p class=" word_wrap">
                          <i class="iconfont2" :style="{color: themeColorShow}" :class="item.subject_type == 5 ? 'icon-circle' : 'icon-xuanze'"></i>{{item2.subject_item_title || '-'}}</p>
                      </div>
                    </div>
                    ...
                    <div v-else-if="item.subject_type == 7 || item.subject_type == 8">
                      <div class="topic-item" v-for="(item2,index2) in item.subject_items" :key="index2">
                        ...
                        <p style="width:100%;" class="word_wrap">
                          <i class="iconfont2" :style="{color: themeColorShow}" :class="item.subject_type == 7 ? 'icon-circle' : 'icon-xuanze'"></i>{{item2.subject_item_title || '-'}}</p>
                      </div>
                    </div>
                    ...
                    <div v-else-if="item.subject_type == 0">
                      ...
                      <div v-if="item.subject_builder.type == 1">
                        <h6>
                          <i class="iconfont2 " :style="{color: themeColorShow}" :class="iconSet[item.subject_builder.builder_id]"></i>{{item.subject_title}}
                          <em class="red">*</em>
                        </h6>
                      ...
                      <div class=" flex-between-wrap" v-else-if="item.subject_builder.type == 2">
                        ...
                        <div class="component-radio flex-start-wrap">
                          <p v-for="_item in item.item_list " :key="_item.val" class=" word_wrap ">
                            <i class="iconfont2 icon-circle" :style="{color: themeColorShow}"></i>{{ _item.desc }}
                          </p>
                        </div>
                      </div>
                      <!-- 多选框 -->
                      <div class=" flex-between-wrap" v-else-if="item.subject_builder.type == 3">
                        ...
                        <div class="component-radio flex-start-wrap">
                          <p v-for="_item in item.item_list " :key="_item.val" class=" word_wrap ">
                            <i class="iconfont2 icon-xuanze" :style="{color: themeColorShow}"></i>{{ _item.desc }}
                          </p>
                        </div>
                      </div>
                      <!--  所在地-->
                ...
                <div class="pcontent-operat">
                  <p v-if="form.allow_anonymous == 1">
                    <i class="iconfont2 icon-xuanze1" :style="{color: themeColorShow}"></i>匿名参加
                  </p>
                  <div class="operat-button no_select_text" :style="{backgroundColor: themeColorShow}">
                    <i class="iconfont2 icon-toupiao"></i>投票
                  </div>
                  <div class="operat-foot">
                    <p>
                      <i class="iconfont2 icon-shouye"></i>首页</p>
                    <p>
                      <i class="iconfont2 icon-fenxiang"></i>分享</p>
                  </div>
                </div>
              </div>
            </div>
    ...
    <el-card class="box-card card-middle" :body-style="{ padding: '90px 0 10px 0',minHeight: '500px', height: 'calc(100vh - 152px)', overflowY: 'auto',overflowX: 'hidden'}">
      <div slot="header" class="clearfix ">
        <span>{{vote_id ? '编辑':'新建' }}投票</span>
      </div>
      <!-- <div style="width:93%;height:74px;line-height:74px;margin: 0 auto 20px;background-color:#39BCF3;color:#fff;">
        <p class="themeHead" style="text-align:center"> {{form.vote_title ? form.vote_title : '标题' }} </p>
      </div> -->
      <div class="card-curBody">
        <el-form label-width="160px" ref="form" :model="form">
          <el-form-item label="投票标题：" prop="vote_title" :rules="[{ required: true, message: '请填写投票标题', trigger: 'blur' }]">
            <el-input key="vote_title" maxlength="40" placeholder="不超过40个字符！" v-model="form.vote_title" style="width:400px;"></el-input>
          </el-form-item>
          <el-form-item label="投票说明：">
            <el-input key="vote_desc" type="textarea" placeholder="不超过200个字符！" maxlength="200" v-model="form.vote_desc" style="width:400px;"></el-input>
          </el-form-item>
          <!-- div.subjects-box 添加指令 v-dragging="{ item: item, list: form.subjects, group: 'subjects', otherData: {index:index} }" 即可拖拽每题 -->
          <div :id="'topic'+index" v-dragging="{ item: item, list: form.subjects, group: 'subjects', otherData: {index:index} }" class="subjects-box" tabindex="-1" v-for="(item,index) in form.subjects" :key="index">
              ...
              <el-form-item label="题目类型：">
                <div style="width:400px;" class="radio-vertical-box">
                  <el-radio-group v-model="item.subject_type">
                    <el-radio label="1">图文单选</el-radio>
                    <el-radio label="2">图文多选</el-radio>
                    <el-radio label="3">文字单选</el-radio>
                    <el-radio label="4">文字多选</el-radio>
                ...
                <div class="topicOption">
                  <div class="topic-option-box">
                    <p v-if="item.subject_type == 1 || item.subject_type == 2">
                      <em class="red">* </em>图片支持、jpg、jpeg、png、gif，建议尺寸500*500
                    </p>
                    <p v-if="item.subject_type == 5 || item.subject_type == 6">
                      <em class="red">* </em>仅支持MP4格式且大小不超过20M的视频
                    </p>
                    <p v-if="item.subject_type == 7 || item.subject_type == 8">
                      <em class="red">* </em>支持MP3、WAV格式且大小不超过10M的音频
                    </p>
                    <div v-for="(item2,index2) in item.subject_items" :key="index2" class="topic-option-item">
                      ...
                      <div v-if="item.subject_type == 1 || item.subject_type == 2">
                        <!-- 上传图片 -->
                        <el-form-item :key="'index'+index+'index2_0_'+index2" label="" :prop="'subjects.'+index+'.subject_items.'+index2+'.subject_item_images[0].img_url'" :rules="[{ required: true, message: '请选择选项图片', trigger: 'change' }]">
                          <uploadFile class="uploadClick avatar-uploader" @change="handleChange" :uploadSelf="true" :maxSize="2" :paramsObj="{indexs:[index,index2],subjectType:'subject_item_images',urlType:'img_url'}">
                            <div v-if="item2.subject_item_images[0].img_url" class="img-bg-box" :style="{backgroundImage: 'url(' + item2.subject_item_images[0].img_url + ')'}">
                            </div>
                            <i v-else class="el-icon-plus avatar-uploader-icon"></i>
                          </uploadFile>
                        </el-form-item>
                        <!-- 选项内容 -->
                        <el-form-item :key="'index'+index+'index2_1_'+index2" style="margin-top:12px" label="" :prop="'subjects.'+index+'.subject_items.'+index2+'.subject_item_title'" :rules="[{ required: true, message: '请填写选项内容', trigger: 'blur' },{ min: 1, max: 8, message: '长度在 1 到 8 个字符', trigger: 'blur' }]">
                          <el-input size="mini" maxlength="8" placeholder="选项内容" v-model="item2.subject_item_title" style="width:130px;"></el-input>
                        </el-form-item>
                      ...
                      <div v-else-if="item.subject_type == 3 || item.subject_type == 4">
                        <!-- 选项内容 -->
                        <el-form-item :key="'index_'+index+'index2_2_'+index2" label-width="80px" :label=" '选项'+(index2+1)+'：'" :prop="'subjects.'+index+'.subject_items.'+index2+'.subject_item_title'" :rules="[{ required: true, message: '请填写选项内容', trigger: 'blur' },{ min: 1, max: 40, message: '长度在 1 到 40 个字符', trigger: 'blur' },{ validator: checkItemName, trigger: 'blur' }]">
                          ...
                          <div class="add-aud-box">
                            <el-form-item :key="'index'+index+'index2_4_'+index2" label="" :prop="'subjects.'+index+'.subject_items.'+index2+'.subject_item_voices[0].aud_url'" :rules="[{ required: true, message: '请选择选项音频', trigger: 'change' }]">
                              <uploadFile class="uploadClick" @change="handleChange" :acceptString="'.mp3,.wav'" :maxSize="10" :paramsObj="{indexs:[index,index2],subjectType:'subject_item_voices',urlType:'aud_url'}">
                                <el-button v-if="item2.subject_item_voices[0].desc == ''" plain size="mini">添加音频</el-button>
                          ...
                          <div class="add-aud-box">
                            <el-form-item :key="'index'+index+'index2_6_'+index2" label="" :prop="'subjects.'+index+'.subject_items.'+index2+'.subject_item_videos[0].vid_url'" :rules="[{ required: true, message: '请选择选项视频', trigger: 'change' }]">
                              <uploadFile class="uploadClick" @change="handleChange" :acceptString="'.mp4'" :maxSize="20" :paramsObj="{indexs:[index,index2],subjectType:'subject_item_videos',urlType:'vid_url'}">
                                <el-button v-if="item2.subject_item_videos[0].desc == ''" plain size="mini">添加视频</el-button>
            ...
            <section v-else>
              ...
              <el-form-item v-if="item.subject_builder.type == 1" :label="item.subject_title+'： '">
                <el-input key="subject_builder_type1" class="component-width" disabled :placeholder="'请输入'+item.subject_title"></el-input>
              </el-form-item>
              <!-- 单选框 -->
              ...
              <el-form-item v-if="item.subject_builder.type == 5" :label="item.subject_title+'： '">
                中国&nbsp;
                <el-cascader placeholder="地区" class="component-width" disabled :options="[]" filterable change-on-select>
                </el-cascader>
              </el-form-item>
              <!-- 文件上传 -->
              ...
              <el-form-item v-if="item.subject_builder.type == 6" :label="item.subject_title+'： '">
                ...
                <el-input key="subject_builder_type6" class="component-width" style="margin-bottom:5px;" disabled placeholder="详细地址"></el-input>
                &nbsp;
                <el-input key="subject_builder_type6_2" style="width:100px;" disabled placeholder="邮政编码"></el-input>
              </el-form-item>
            </section>
          ...
          <!-- <el-form-item label="投票截止时间：" prop="end_date" :rules="[{ required: true, message: '请选择截止时间', trigger: 'blur' }]">
          ...
          <!-- <el-form-item label="投放区域： " prop="service_id" :rules="[{ required: true, message: '请选择投放区域', trigger: 'blur' }] ">
          <get-service-id @changeServiceId="changeServiceId "></get-service-id>
        </el-form-item> -->
          ...
          <el-form-item style="margin-top:30px;">
            <el-button :loading="submitStatus1" v-if="!vote_id" type="primary" @click="onSubmit( 'form','1') ">发布</el-button>
    ...
    <el-card class="box-card card-right " :body-style="{  minHeight: '550px', height: 'calc(100vh - 150px)',overflowY: 'auto', overflowX: 'hidden',padding: '98px 15px 20px'}">
      <div slot="header" class="clearfix">
        <span>设置</span>
      </div>
      <el-collapse accordion value="2">
        ...
        <el-collapse-item title="联系人组件" name="2">
          <div style="height: calc(100vh - 150px - 140px  - 48px * 4);overflow-y:auto;min-height:315px;">
            <checkCard ref="contactConp" @change="changeCard" multiple :row="3" :isShowIcon="false" :iconSize="42" :dataKey="{desc: 'builder_name',value:'builder_icon'}" :componentColor="'#fff'" :itemColor="'#fbfbfb'" :paddingW="22" :checkArrP="checkArr"></checkCard>
          </div>
        </el-collapse-item>
...
import { RequestServiceRun, RequestVoteAdd, RequestSubjectList, RequestVoteEdit, RequestSubjectRemove, RequestSubjectItemRemove, RequestVoteDetail, RequestServiceIcon, RequestServiceClassAdd, RequestformBuilderList, RequestThemeList } from "@/fetch/modules/service.js";
import { RequestShareTheme } from "@/fetch/modules/share.js";
// 多选的组件ID
const _multipleComponent = ["4000002", "4000003", "4000012", "4000013", "4000014", "4000015"];
//十六进制颜色值的正则表达式
var reg = /^#([0-9a-fA-f]{3}|[0-9a-fA-f]{6})$/;
/*16进制颜色转为RGBA格式*/
String.prototype.colorRgba = function(_a) {
  ...
  if (sColor && reg.test(sColor)) {
    ...
    return "RGB(" + sColorChange.join(",") + "," + _a + ")";
  } else {
    return sColor;
  }
};
...
export default {
  ...
  beforeRouteEnter(to, from, next) {
    next(vm => {
      ...
      console.log("urlLastId", to.meta.id);
      console.log("__beforeRouteEnter__", vm.$route.query.vote_id);
    });
  },
  created() {
    this.getMpactivity();
  ...
  methods: {
    ...
    changeCard(_indexs, _data, _params) {
      ...
      if (_params.checked) {
        ...
        _subjects.some((item, index) => {
          if (item.subject_type == "0" && item.subject_builder.builder_id == _componentID) {
    ...
    toBottom() {
      setTimeout(() => {
        ...
        // 题目滚动
        const _middleBody = document.querySelector(".card-middle .el-card__body");
    ...
    getShareTheme() {
      RequestShareTheme({ theme_type: 3 }).then(res => {
    ...
    getThemeList() {
      RequestThemeList().then(res => {
        ...
        if (res.code === 2000) {
          ...
          console.log("_colorArr", _colorArr);
          this.checkThemeColor = _colorArr;
          this.checkThemeImg = _imgArr;
        }

        // this.checkThemeColor = this.checkThemeColor.concat(_myColor);
    ...
    changeTheme(_index, _data, params) {
      ...
      // console.log("this.form.theme_id", this.form.theme_id, params.id);

      // 主题ID
      this.form.theme_id = _index.length ? _data.theme_id : "";

      // this.themeControl(_data.theme_color, _data.theme_bg);
      // 主题颜色
      this.themeColorShow = _data.theme_color || "#39bcf3";
      // 主题背景
      document.querySelector(".phone-box").style.backgroundImage = _data.theme_bg ? "url(" + _data.theme_bg + ")" : "";
    },
    // 主题控制
    themeControl(_color, _img) {
      ...
      if (isSupported) {
        ...
        document.body.style.setProperty("--theme-color", _color.colorRgba(1));
        ...
        /* not supported */
        document.querySelector(".phone-box .phone-head").style.backgroundColor = _color;
        document.querySelector(".phone-box .operat-button").style.backgroundColor = _color;

        [].forEach.call(document.querySelectorAll(".phone-content  .iconfont2"), function(item) {
          item.style.color = _color;
        });
        [].forEach.call(document.querySelectorAll(".css-triangle"), function(item) {
          item.style.borderTop = "4px solid " + _color;
        });
        [].forEach.call(document.querySelectorAll(".component-input"), function(item) {
          item.style.borderBottom = "1px solid " + _color;
        });
      }

      // 背景
      document.querySelector(".phone-box").style.backgroundImage = _img ? "url(" + _img + ")" : "";
    },
    ...
    checkItemName(rule, value, callback) {
      let _index = rule.field.split(/subjects.|.subject_items.|.subject_item_title/);
      ...
      RequestSubjectList({
        ...
        this.mySubjects.map((item, index) => {
          ...
          if (item.subject_type == "0") {
            // subject_builder_list 字段  =>  subject_builder 字段
            ...
            /* newAdd */
            if (!this.isInArr(_subject_builder.builder_id, _multipleComponent)) {
              // 找到组件 index
    ...
    removeTopic(index, _data) {
      if (this.form.subjects.length > 1) {
        console.log("移除题目：", index, _data);
        // console.log("asdas", this.vote_id, this.form.subjects[index].subject_id);
        let _subject_id = this.form.subjects[index].subject_id;
        if (this.vote_id && _subject_id) {
          // 编辑投票
          RequestSubjectRemove({
            ...
            if (res.code == 2000) {
              ...
              if (_data.subject_builder) {
                const _itemIndex = this.componentIdIndex[_data.subject_builder.builder_id];
          ...
          // 组件 /* newAdd */
          if (_data.subject_builder) {
            let _itemIndex = this.componentIdIndex[_data.subject_builder.builder_id];
            console.log("_itemIndex", _itemIndex);
            // 取消选择状态
            this.$refs["contactConp"].cancelCheck([_itemIndex]);
          }
    ...
    addOption(index) {
      ...
      console.log("subjects", this.form.subjects[index].subject_items);
      this.form.subjects[index].max_select = this.form.subjects[index].subject_items.length + "";
    },
    // 移除选项
    removeOption(index, index2) {
      if (this.form.subjects[index].subject_items.length > 1) {
        let _subject_item_id = this.form.subjects[index].subject_items[index2].subject_item_id;
              ...
              this.$refs[formName].validate(valid => {
                ...
                if (valid) {
                  ...
                  _args.subjects.map(function(item) {
                    ...
                    if (item.subject_type != "1" && item.subject_type != "2" && item.subject_items) {
                      item.subject_items.map(function(sj_items) {
                        delete sj_items.subject_item_images; /* 删除选项图片 */
                        return sj_items;
                      });
                    }
                    if (item.subject_type != "5" && item.subject_type != "6" && item.subject_items) {
                      item.subject_items.map(function(sj_items) {
                        delete sj_items.subject_item_videos; /* 删除选项视频 */
                        return sj_items;
                      });
                    }
                    if (item.subject_type != "7" && item.subject_type != "8" && item.subject_items) {
                      item.subject_items.map(function(sj_items) {
                  ...
                  _args.share_desc = _args.share_desc || _args.vote_title;
                  // return;
                  type == "1" ? (this.submitStatus1 = true) : (this.submitStatus2 = true);
                  this.submitStatus1;
                  if (!this.vote_id) {
                    RequestVoteAdd(_args).then(res => {
                      type == "1" ? (this.submitStatus1 = false) : (this.submitStatus2 = false);
                      if (res.code == 2000) {
                        this.backToList();
                    ...
                    _args.vote_id = this.vote_id;
                    RequestVoteEdit(_args).then(res => {
                      type == "1" ? (this.submitStatus1 = false) : (this.submitStatus2 = false);
                      console.log("__RequestVoteEdit__", res);
                      if (res.code == 2000) {
                        this.backToList();
                      } else if (res.code == 4000) {
                        this.$message({
    ...
    previewVote() {
      let _args = JSON.parse(JSON.stringify(this.form));
      this.$router.push({
        path: "previewVotes" + this.urlLastId,
        query: { data: JSON.stringify(_args) }
      });
    },
    backToList() {
      const _url = !this.vote_id ? "/addVote" : "/editVote";
      this.$router.push({
        path: "vote" + this.urlLastId
      });
      this.$store.dispatch("delVisitedViews", {
        path: "/voting" + this.urlLastId + _url + this.urlLastId
      });
    }
  },
  destroyed() {}
...
const _checkArr = [
  {
    ...
    builder_icon: "https://img.sqydt.darongshutech.com/image_201809211010133261.jpg"
  },
  {
    builder_name: "手机",
    builder_id: "002",
    ...
    builder_icon: "https://img.sqydt.darongshutech.com/image_201809211016395821.jpg"
  },
  {
    builder_name: "邮箱",
    builder_id: "003",
    ...
    builder_icon: "https://img.sqydt.darongshutech.com/image_201809211016437367.jpg"
  },
  {
    builder_name: "公司",
    builder_id: "004",
    ...
    builder_icon: "https://img.sqydt.darongshutech.com/image_201809211016475679.jpg"
  },
  {
    builder_name: "部门",
    builder_id: "005",
