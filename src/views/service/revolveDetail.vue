      ...
      <div v-if="dialogVisible">
        <el-dialog title="分享样式设置" :visible.sync="dialogVisible" width="60%" :close-on-click-modal="false">
          <el-form label-width="150px" :close-on-click-modal="false">
            <share-template :share_desc="form1.share_desc" :share_pic="form1.share_pic" :markBtn='true' :curTitle="curTitle" @submitForm="submitFormShare" :templateData="templateData" @cancleForm="dialogVisible=false;"></share-template>
          </el-form>
        </el-dialog>
      </div>
...
export default {
  ...
  created() {
    this.urlLastId = this.$route.query.id;
    this.needAudit = this.$route.query.need_audit;
    this.search.service_id = this.$route.query.service_id;
    this.addForm.service_id = this.$route.query.service_id;
    this.search.need_audit = this.$route.query.need_audit;
    this.form.service_id = this.$route.query.service_id;
    this.newList();
    this.getClass();
  ...
  methods: {
    ...
    submitFormShare(obj) {
      var args = {
        service_id: this.search.service_id,
        theme_type: this.form1.theme_type,
    ...
    //获取活动详情
    getActivityDetail() {
      RequestActivityInformation({ service_id: this.addForm.service_id }).then(
        res => {
          this.dialogRelateActive = true;
          setTimeout(() => {
          ...
          if (res.link_third_miniprogram) {
            res.link_third_miniprogram.forEach((item, index) => {
              this.mpActiveData.forEach((child, num) => {
                if (
                  item.service_id == child.service_id &&
                  item.branch_id == child.branch_id
                ) {
                  setTimeout(() => {
    ...
    changeShowInUser(val) {
      if (val) {
        this.$confirm("开启后当前模块将在用户端显示?", "提示", {
          ...
          .then(() => {
            RequestServiceState({ service_id: this.search.service_id }).then(
              res => {
                if (res.code == 2000) {
    ...
    handleCase(case_id, node) {
      ...
      let title =
        node != 0
          ? "是否结案当前爆料？"
          : "当前爆料无节点记录,至少添加一个节点可结案是否跳转至添加节点页面？";
