      ...
      <el-card class="box-card">
        <div class="table-list">
          ...
          <div class="table-title">
            <div>
              <router-link :to="{path:'activeList'+urlLastId}">
                <el-button type="primary">返回活动列表</el-button>
              </router-link>
          ...
          <el-table :data="tableData" border style="width: 100%" ref="table" v-loading='loading' element-loading-text="拼命加载中...." element-loading-spinner="el-icon-loading" @row-click="handleCurrentChange" @selection-change="selectionChange">
            ...
            <el-table-column prop="news_title" label="活动标题" align="center">
              <template slot-scope="scope">
                <span style="color:#409eff">
                  <router-link class="news-link" :to="{path:'activeDetail'+urlLastId,query:{activity_id:scope.row.activity_id,active:2}}">{{scope.row.activity_title}}</router-link>
                </span>
            ...
            <el-table-column label="操作" width="100" align="center" v-if="search.audit_status!=3">
              <template slot-scope="scope">
                <router-link v-if="active==2 || active==3" :to="{path:'lookSign'+urlLastId,query:{activity_id:scope.row.activity_id}}">
                  <el-button type="text" size="small"> 查看报名 </el-button>
                </router-link>
                <el-button type="text" size="small" v-if="active==1" @click="submit(scope.row)"> 发布 </el-button>
                <router-link v-if="active==1||active==3" :to="{path:'activeEditor'+urlLastId,query:{activity_id:scope.row.activity_id,atcive:'2',status:0}}">
                  <el-button type="text" size="small"> 编辑 </el-button>
                </router-link>
                <el-button type="text" size="small" v-if="active==1" @click="handDelete(scope.row)"> 删除 </el-button>
                ...
                <!-- <div class="el-icon-wrapper">
                  ...
                  <i class="icon-fabu" v-if="active==1" @click="submit(scope.row)" title="发布"></i>
                  <i class="el-icon-delete" type="text" size="small" v-if="active==1" @click="handDelete(scope.row)" title="删除"></i>
                </div> -->
              </template>
            </el-table-column>
          </el-table>
...
// '列表，分类，删除',发布
import { RequestActiveList, RequestActiveAttr, RequestActiveDelete, RequestActivieState } from "@/fetch/modules/service.js";
// 正则匹配
const rules = {
  news_title: [{ required: true, message: "请输入资讯标题", trigger: "blur" }, { min: 1, max: 40, message: "长度在 1 到 40 个字符", trigger: "blur" }]
};
export default {
  ...
  data() {
    return {
      ...
      selectionValues: [],
      switchData: [{ name: "未开始", active: "1" }, { name: "进行中", active: "2" }, { name: "已结束", active: "3" }], // 上线下线,
      active: 1,
      loading: false,
