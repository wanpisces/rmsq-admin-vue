      ...
      <div class="main-board-wrapper">
        ...
        <div v-for="(item,idx) in  dataStream.newspaper_page_items" :key="idx" @click="handleTransform(item,idx)" style="cursor: pointer" v-if="item.item_type!=0">
          ...
          <div :style="{position:'absolute',overflow:'hidden', top:`${item.points.coords[1]}px`,left:`${item.points.coords[0]}px`,width:`${item.points.coords[2]-item.points.coords[0]}px`, height:`${item.points.coords[3]-item.points.coords[1]}px`, border:builderId==item.builder_id?'2px dotted #39bcf3':'2px dotted #fff'}">
            <section class="one" v-if="item.builder_id==22">
              <div class="board-bg" :style="{backgroundImage: `url(${item.item_images.length? baseUrl+item.item_images[0] : ''})`,fontSize:'0px',height:'315px'}">
              </div>
              <article style="padding:10px 25px;line-height:23px;font-size:12px;background:#779FC9;color:#fff;">
                <div style="height:140px;overflow: hidden" v-html="item.item_txt">
                </div>
              </article>
            </section>

            <section v-else-if="item.builder_id==23">
              <div class="board-title" style="margin-bottom:15px;height:46px">
                <div class="line_ellipsis_2">{{item.item_title}}</div>
              </div>
              <article style="line-height:23px;font-size:12px;height:140px;overflow: hidden;">
                <div v-html="item.item_txt"></div>
              </article>
              <div class="board-build3" :style="{backgroundImage: `url(${item.item_images.length? baseUrl+item.item_images[0] : ''})`,fontSize:'0px',height:'160px'}"></div>
            </section>

            <section v-else-if="item.builder_id==24">
              <div class="board-title  " style="margin-bottom:15px;height:46px">
                <div class="line_ellipsis_2">{{item.item_title}}</div>
              </div>
              <div class="board-build3" :style="{backgroundImage: `url(${item.item_images.length? baseUrl+item.item_images[0] : ''})`,fontSize:'0px',height:'160px'}"></div>
              <article style="line-height:24px;font-size:12px;height:505px;overflow: hidden;margin-top:10px">
                <div v-html="item.item_txt"></div>
              </article>
            </section>

            <section v-else-if="item.builder_id==25">
              <div class="board-title" style="margin-bottom:15px;height:46px">
                <div class="board-title-border"></div>
                <div class="line_ellipsis_2">{{item.item_title}}</div>
              </div>
              <article style="line-height:26px;font-size:12px;height:313px;overflow: hidden;">
                <div v-html="item.item_txt"></div>
              </article>
            </section>

            <section v-else-if="item.builder_id==26">
              <div class="board-title" style="margin-bottom:15px;height:46px">
                <div class="board-title-border"></div>
                <div class="line_ellipsis_2">{{item.item_title}}</div>
              </div>
              <article style="line-height:25px;font-size:12px;height:428px;overflow: hidden;">
                <div v-html="item.item_txt"></div>
              </article>
            </section>

            <section v-else-if="item.builder_id==27">
              <div class="board-com-6 ">
                <div class="ellipsos">
                  {{item.item_title}}
                </div>
              </div>
              <div class="board-flex">
                ...
                <div class="board-build3" :style="{backgroundImage: `url(${item.item_images.length? baseUrl+item.item_images[0] : ''})`,fontSize:'0px',height:'180px',width:'200px',marginLeft:'20px',flexShrink:'0'}"></div>
              </div>

            </section>
            <section v-else-if="item.builder_id==28">
              <div class="board-com-7" style="height:81px">
                <div class="ellipsos">{{item.item_title}}</div>
                <!-- <div class="line_ellipsis_2">{{item.item_subtitle}}</div> -->
                <div style="line-height:18px;font-size:12px;overflow: hidden;height:38px;font-weight: normal;" class="line_ellipsis_2">
                  <div v-html="item.item_txt"></div>
                </div>
              </div>
              <div class="board-build3" :style="{backgroundImage: `url(${item.item_images.length? baseUrl+item.item_images[0] : ''})`,fontSize:'0px',height:'330px'}"></div>
            </section>
          </div>
