<template>
  <div class="my-container sheQuBao-body">
            ...
            <el-collapse-item title="版面信息" name="0">
              <div>
                <el-form-item label="版面名称">
                  <el-input v-model="form.newspaper_page.page_header" placeholder="请输入版面名称" :maxlength="9"></el-input>
                </el-form-item>
                <el-form-item label="报尾">
                  <el-input v-model="form.newspaper_page.page_end" placeholder="请输入报尾" :maxlength="50"></el-input>
                </el-form-item>
              </div>
            </el-collapse-item>
            <!-- 构件信息 -->
            <el-collapse-item :title="formControl.buildWb[formControl.curBuildT]+'信息'" name="1">
              <div :style="layout.collapseStyle">
                <el-scrollbar ref="elscrollbarBuild" style="height:100%;">
                  ...
                  <!-- 报纸标题 （ 只有 报头构件 有 ）-->
                  <!-- <el-form-item v-if="(formControl.curBuildT == 0 && formControl.titleType == '1')" label="报纸标题" :prop="'newspaper_page_items['+formControl.indexControl+'].item_title'" :rules="[{ min: form.newspaper_page_items[formControl.indexControl].rule.title_min_len , max: form.newspaper_page_items[formControl.indexControl].rule.title_max_len , message: '长度在 '+ form.newspaper_page_items[formControl.indexControl].rule.title_min_len+' 到 '+form.newspaper_page_items[formControl.indexControl].rule.title_max_len+' 个字符', trigger: 'blur' }]">
                    <el-input v-model="form.newspaper_page_items[formControl.indexControl].item_title" placeholder="请输入报纸标题"></el-input>
                  </el-form-item> -->

                  <!-- 构建标题 -->
                  <!-- <el-form-item v-if="formControl.curBuildT != 0" label="标题" :prop="'newspaper_page_items['+formControl.indexControl+'].item_title'" :rules="[{ min: 10, max:  50, message: '长度在 10 到 50 个字符', trigger: 'blur' }]">
                  ...
                  <el-form-item v-if="formControl.curBuildT == '0' && formControl.titleType == '1'" label="副标题">
                    <el-input maxlength="13" v-model="form.newspaper_page_items[formControl.indexControl].item_subtitle" placeholder="请输入副标题" :max="13"></el-input>
                  </el-form-item>

                  <!-- 内容 （除了 报头构件，其他都有） -->
                  <el-form-item v-if="formControl.curBuildT != 0" label="内容">
                    <div class="build-content word_wrap" @click="formControl.editorShow = true">
                      {{itemText}}
                    </div>
                  </el-form-item>
                  ...
                  <!-- 广告跳转链接 （只有 广告构件 有）-->
                  <!-- <el-form-item v-if="formControl.curBuildT == 3" label="广告跳转链接">
                    <el-input v-model="form.newspaper_page_items[formControl.indexControl].item_adv" placeholder="请输入广告跳转链接"></el-input>
                  </el-form-item> -->
    ...
    <div class="layout-foot">
      <el-button @click="toPreviousPlate">{{ formControl.stepIndex == 1 ? '返回上一版' : '返回上一版' }}</el-button>
    ...
    <!-- 编辑内容 -->
    <el-dialog @open="openEditor" :modal-append-to-body="false" title="编辑内容" :close-on-click-modal="false" :visible.sync="formControl.editorShow" width="40%" :before-close="closeEditor">
      ...
      <span slot="footer" class="dialog-footer">
        <el-button @click="formControl.editorShow = false">取 消</el-button>
...
export default {
  ...
  methods: {
    ...
    // 打开内容编辑
    openEditor() {
      // this.$refs["sqbTinymce"].setContent(this.form.newspaper_page_items[this.formControl.indexControl].item_content);
    },
    toTop() {
      // 滚动条
    ...
    // 编辑完成
    sureEditor() {
      if (this.checkFontLength("item_content")) {
    ...
    checkFontLength(filed) {
      ...
      const _fieldName = _isTitle ?  this.formControl.curBuildT == 0 ? "报纸标题" : "标题" : "内容";
      ...
      // 检查内容的字数
      if (_thisData.rule) {
        const _len = _isTitle ? _thisData.item_title.length : _thisData.item_content.replace(/<[^>]*>|[\r\n\s]*/gi, "").replace(/&nbsp;/gi, "").length;
      ...
      if (!checkResult) {
        this.$message({
          type: "warning",
          message: _fieldName + (_thisData.rule[_maxField] ? "应该为 " + _thisData.rule[_minField] + " - " + _thisData.rule[_maxField] + " 个字！" : "至少为 " + _thisData.rule[_minField] + " 个字！")
        });
      }
      return checkResult;
    },
    ...
    registerComponent(temId) {
      return import(`./../../plate/index.vue`).then(component => {
    ...
    submitForm(callback) {
      ...
      console.log(this.form);
      _args.newspaper_page_items.map((item, index) => {
        if (item.item_type == 3) {
