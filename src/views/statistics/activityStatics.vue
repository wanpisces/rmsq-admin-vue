    ...
    <div class="line-wrapper">
      ...
      <div class="nav-wrapper">
        ...
        <ul class="count-time" v-if="adminType==0">
          <li @click="tabUserchanges(item.activityType)" :class="search.activityType==item.activityType?'zuobiao bgColor':false" v-for="(item,index) in tabUser" :key="index">{{item.name}}</li>
        </ul>
        <div class="line-nav">
          <ul class="count-time">
...
export default {
  ...
  data() {
    return {
      ...
      tabUser: [
        { activityType: 1, name: "举办活动数量" },
        { activityType: 2, name: "活动报名人数" }
      ],
  ...
  methods: {
    getChart() {
      ...
      HomeRequst.RequesCountActivity(args).then(res => {
        if (res.code === 2000) {
    ...
    addCountInit(data) {
      ...
      var activityType =
        this.search.activityType == 1 ? "举办活动数量" : "活动报名人数";
      var type = this.search.type == 1 ? "累计" : "新增";
      var titleCurser = type + activityType + "总数";
      let args = {
        type: this.show
      };
      chart.setOption({
    ...
    tabUserchanges(activityType) {
      this.search.activityType = activityType;
      this.getChart();
    },
    tabAddchanges(type) {
