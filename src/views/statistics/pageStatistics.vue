<template>
  <div class="scrollbar">
    <div class="line-wrapper">
      <div class="use-wrap">
        <router-link style="margin-right:20px;" :to="{name:'pageStatisticsDetail'}">页面功能统计详情</router-link>
        <router-link :to="{name:'toolClickStatics'}">工具使用情况</router-link>
      </div>
      ...
      <div>
        <div id="echartContainer1" style="width:100%;height:500px;margin-top: 40px"></div>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    getChart() {
      ...
      HomeRequst.RequesCountModule(args).then(res => {
        if (res.code === 2000) {
    ...
    // 用户增量统计
    addCountInit(data) {
      var chart = echarts.init(document.getElementById("echartContainer1"));
    ...
    tabUserchanges(moduleType) {
      this.search.moduleType = moduleType;
      this.getChart();
    },
    tabAddchanges(type) {
