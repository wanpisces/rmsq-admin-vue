  ...
  <div class="info-container">
    <el-card>
      <el-button icon="el-icon-arrow-left" @click="callback">
        <router-link class="news-link" :to="{ name: 'infoList'}"> 返回审核列表 </router-link>
      </el-button>
      <div class="news-main">
...
import { checkURLTwo } from "../../fetch/tool";
export default {
  data() {
    return {
      newsData: {
  ...
  methods: {
    callback() {
      this.$router.push({ path: "infoExamine" });
      this.$store.dispatch("delVisitedViews", {
        path: "/information/inforExamineDetail"
      });
    }
  }
};
