      ...
      <div style="width:500px;margin:20px auto">
        <el-form :model="oridary" ref="addItemData" label-width="170px" class="demo-ruleForm">
          <el-form-item label="是否同步至微信公众号:" prop="to_mp" :rules="[ { required: true, message: '请选择活动资源', trigger: 'change' }]">
            <el-radio-group v-model="oridary.to_mp">
          ...
          <div v-if="oridary.to_mp==1  ">
            ...
            <!-- </el-form-item> -->
            <div class="form_tips" v-if="oridary.to_mp==1">温馨提示:公众号每个月发文次数有限，服务号一个月仅可发布4次，订阅号一天只能发1次，未发布成功，说明次数已用完</div>
          </div>
    ...
    <section>
      ...
      <div class="editor_container">
            ...
            <el-form label-position="top" :model="addItemData[activeItem]" ref="addItemData">
              <el-form-item label="选择资讯类型" prop="news_type" :rules="[ { required: true, message: '请选择活动资源', trigger: 'change' }]">
                <el-radio-group v-model="addItemData[activeItem].news_type">
                  ...
                  <el-radio label="2" :disabled="oridary.to_mp==1?true:false">图片资讯</el-radio>
                  <el-radio label="3" :disabled="oridary.to_mp==1?true:false">视频资讯</el-radio>
                </el-radio-group>
              </el-form-item>
        ...
        <div v-if="addItemData[activeItem].news_type==1" style="position:relative">
          ...
          <kind-editor id="editor_id" ref="editor_id" :items="kindEditorItems" height="700px" width="100%" :content="addItemData[activeItem].news_content" :afterChange="afterChange" pluginsPath="/static/kindeditor/plugins/" :loadStyleMode="false" :uploadJson="uploadJson" :isVideioShow='true' :isImgShow='true' @on-content-change="onContentChange"  ></kind-editor>
        </div>
              ...
              <div v-if=" addItemData[activeItem].run_in!=2 ">
                <el-form-item label="资讯封面类型">
                  <el-radio-group v-model="addItemData[activeItem].news_pic_type">
                    <el-radio label="0" :disabled="oridary.to_mp==1?true:false">无图</el-radio>
            ...
            <el-radio-group v-model="oridary.show_at">
              <el-radio label="1">原生样式
                <el-tooltip class="item" effect="dark" placement="bottom-start">
                  ...
                  <img src="../../../../static/images/notice (2).png" width="20px" />
                </el-tooltip>
              </el-radio>
              <el-radio label="2">
                h5样式
...
import { RequestMpActive, RequestUploads, RequestDraftList, RequestDraftfoAdd } from "@/fetch/modules/public";
// 存储的cookies
...
export default {
  components: { UE, loadImage, loadVedio, UpLoadImage, iphoneView, kindEditor },
  data() {
    return {
      ...
      uploadJson: "api/public/kindEditor/uploads", // 富文本编辑器
  ...
  methods: {
    ...
    // kindEdior
    afterChange(params) {
      // console.log("afterChange", params);
    },
    // 资讯正文
    onContentChange(params) {
      this.addItemData[this.activeItem].news_content = params;
    },

    saveDraft() {
      let _this = this;
      if (this.addItemData[0].news_title || this.addItemData[0].news_content) {
        RequestDraftfoAdd({ content: { addItemData: this.addItemData, oridary: this.oridary }, type: 1 }).then(res => {
          console.log(res);
        });
      }
    },
    ...
    // 获取公众号列表
    getMpData() {
      RequestMpList({ current_page: 1, page_size: 1000, q: this.search.m }).then(res => {
        if (res.code === 2000) {
    ...
    hanldeCategory(val) {
      ...
      let args = { category_id: this.addItemData[this.activeItem].category_id, pid: 0 };
      this.getClassification(args);
    },
    // 获取活动列表
    getMpactivity() {
      RequestMpActive({ current_page: 1, page_size: 1000, q: this.search.q }).then(res => {
        if (res) {
    ...
    getModule() {
      RequestModuleServiceList({ type: 2 }).then(res => {
        ...
        if (this.addItemData[this.activeItem].category_id) {
          this.getClassification({ category_id: this.addItemData[this.activeItem].category_id, pid: 0 });
        }
      });
    },
    // 获取 栏目模块
    ...
    handleChoose() {
      ...
      if (this.imageType == 0) {
        ...
        let _insertContent =
          this.imageType == 1 ? `<img  width="100%" src=' ${this.baseUrl + this.imgChooseData.material_url}' alt='文章图片'>` : `<div><video  width="100%" src="${this.baseUrl + this.videoChooseData.material_url}" controls="controls">${this.videoChooseData.material_name || "开发者"}</video> </div>`;
        // this.$refs.ue.clearEditor(); // 在光标处插入内容前清空提示
        // this.$refs.editor_id.insertHtml(_insertContent); // 在光标处插入内容 （图片/视频等 ）
        this.$refs.editor_id.insertContent(_insertContent)
        // console.log()
        this.handleClose();
      }
    },
