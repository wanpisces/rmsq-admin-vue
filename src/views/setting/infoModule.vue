<template>
  <div class="info-container">
    ...
    <el-card class="box-card">
      <div class="table-list">
        <div class="table-title">
          <div>
            <el-button type="danger" v-if="perm.cateDele==1" plain @click="tabelDelete" :disabled="deleteMark">删除</el-button>
        ...
        <el-table :data="tableData" border @selection-change="handleSelectionChange" style="width: 100%">
          ...
          <el-table-column label="接入链接" width="100px" v-if="perm.linkCopy==1">
            <template slot-scope="scope">
              <!-- <el-button ref="link" size="small" @click="copyText($event)" :data-text="scope.row.wechat_connect_wxapp_link" style="width:68px;overflow: hidden;">
                复制链接
                <span>{{scope.row.wechat_connect_wxapp_link}}</span>
              </el-button> -->
              <div @click="copyText($event)" style="cursor:pointer;border:1px  solid #e6e6e6;padding:2px;text-align:center;font-size:14px" :data-text="scope.row.third_link">
                复制链接
              </div>
            </template>
          </el-table-column>
          <el-table-column fixed="right" label="操作" width="100" v-if="perm.cateEdit==1||perm.cateState==1">
            <template slot-scope="scope">
              <el-button type="text" size="small" v-if="perm.cateEdit==1" @click="handEditor(scope.row)">编辑</el-button>
              <el-button type="text" size="small" v-if="perm.cateState==1" @click="onOfflineClick(scope.row)">{{active == 1?"停用":"启用"}}</el-button>
              <!-- <div class="el-icon-wrapper">
                <i class="el-icon-edit" @click="handEditor(scope.row)" title="编辑" v-if="perm.cateEdit==1"></i>
                <i :class="active == 1?'icon-ban':'icon-qiyong'" @click="onOfflineClick(scope.row)" v-if="perm.cateState==1" :title="active == 1?'禁用':'启用'"></i>
              </div> -->
            </template>
          </el-table-column>
        </el-table>
    ...
    <textarea id="input">这是幕后小黑手</textarea>
    <!-- 编辑 -->
  </div>
</template>
<script>
