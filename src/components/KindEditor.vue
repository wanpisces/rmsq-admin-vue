<template>
  <div class="kindeditor-component">
    <textarea :ref="id" :id="id" name="content" v-model="outContent" class="kind-editor"></textarea>
    <div class="insert-box">
      <editor-image color="#20a0ff" class="editor-upload-btn" @successCBK="imageSuccessCBK" v-if="isImgShow">插入图片</editor-image>
      <editor-vedio style="margin-left:10px;" color="#20a0ff" class="editor-upload-btn" @successCBK="vedioSuccessCBK" v-if="isVideioShow">插入视频</editor-vedio>
    </div>
  </div>
</template>
...
// Vue.use(Element).use(VueKindEditor);
import editorImage from "./Tinymce/components/editorImage.vue";
import editorVedio from "./Tinymce/components/editorVedio.vue";
export default {
    ...
    htmlTags: {
      ...
      default: function() {
        return {
          ...
          section: ["width", "height", "style", "align"],
          img: ["src", "width", "height", "border", "alt", "title", "align", "style", "/"],
          video: ["src", "width", "height", "class", "controls", "title", "style", "/"],
          hr: ["class", "/"],
          br: ["/"],
  ...
  methods: {
    ...
    imageSuccessCBK(arr) {
      const _this = this;
      arr.forEach(v => {
        this.insertContent(`<img src="${v.url}" style="width:100%" >`);
      }, this);
    },
    vedioSuccessCBK(url) {
      this.insertContent(` <video src="${url}" controls="controls" width="100%"></video>`);
    }
  }
};
</script>
